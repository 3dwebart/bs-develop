    ...
    <section id="sit_ov">
        ...
        */
        ...
        <div id="sit_ov_btn">
            ...
            <?php } ?>
            <a href="javascript:item_wish('<?php echo $it['it_id']; ?>');" id="sit_btn_wish">위시리스트</a>
        ...
        $(function(){
            // 위시리스트 레이어 닫기
            $("#wish_msg_close, #wish_msg_no").on("click", function() {
            ...
            // 위시리스트 이동
            $("#wish_msg_yes").on("click", function() {
...
// 바로구매, 장바구니 폼 전송
// size() 함수는 jquery 3.0 이후 length 로 통합됨 - 변경 : 2018-09-27
function fitem_submit(f)
{
    ...
    if($(".sit_opt_list").length < 1) { // size() ==> length
    ...
    if (document.pressed == "장바구니") {
        $.post(
            "<?php echo G5_SHOP_CSS_URL; ?>/item.cartupdate.php",
            $(f).serialize(),
            function(error) {
                if(error != "OK") {
                    alert(error.replace(/\\n/g, "\n"));
                ...
                var top = pos.top + $("#sit_btn_cart").height() + 10;
