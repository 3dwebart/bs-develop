...
$(function() {
	...
	$(document).on("keyup", "select.it_option", function(e) {
		var sel_count = $("select.it_option").length; // size( ) 3.0 이후부터 사라짐
		...
		switch(mode) {
			...
			case "삭제":
				if(confirm("선택하신 옵션항목을 삭제하시겠습니까?")) {
					...
					if($("#sit_sel_option .sit_spl_list").length > 0) { // size( ) 3.0 이후부터 사라짐
						// 선택옵션이 하나이상인지
...
function add_sel_option(type, id, option, price, stock)
{
	...
	if($("#sit_sel_option > ul").length < 1) { // size( ) 3.0 이후부터 사라짐
		...
		if(type) {
			if($("#sit_sel_option .sit_spl_list").length > 0) { // size( ) 3.0 이후부터 사라짐
				...
				if($("#sit_sel_option .sit_opt_list").length > 0) { // size( ) 3.0 이후부터 사라짐
...
{
	var result = false;
	var listOn = $('select.it_option.list-on').length;
